...
func DefaultSchedulerOptions(name string) *SchedulerOptions {
...
type LockProvider interface {
	// CreateLock 创建锁实例
	CreateLock(ctx context.Context, key string, options ...lock.LockOption) (lock.Lock, error)
}
...
// NewScheduler 创建调度器
func NewScheduler(lockProvider LockProvider, options *SchedulerOptions) (*Scheduler, error) {
	if options == nil {
...
func (s *Scheduler) Name() string {
	return consts.ComponentScheduler
}

// Status 返回组件状态
func (s *Scheduler) Status() consts.ComponentStatus {
	return s.status
}

// Init 初始化组件
func (s *Scheduler) Init(baseConfig *config.BaseConfig, cfgBody []byte) error {
...
func (s *Scheduler) Start(ctx context.Context) error {
	if s.status != consts.StatusInitialized {
		return fmt.Errorf("调度器未初始化或已启动")
	}

	// 如果设置了锁名称，尝试获取分布式锁
	if s.lockName != "" && s.lockProvider != nil {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		lock, err := s.lockProvider.CreateLock(ctx, s.lockName, lock.WithLockTTL(int(s.options.LockTTL.Seconds())))
		cancel()
...
func (s *Scheduler) Stop(ctx context.Context) error {
	if s.status != consts.StatusRunning {
		return nil
	}
...
// RegisterMetadata 注册服务元数据
func (s *Scheduler) RegisterMetadata() (bool, int, map[string]string) {
	return false, 0, nil
}

// DefaultConfig 返回默认配置
func (s *Scheduler) DefaultConfig(baseConfig *config.BaseConfig) interface{} {
	return &SchedulerOptions{
		LockTTL:     10 * time.Second,
		TaskTimeout: 5 * time.Minute,
		LockName:    "scheduler",
...
func (s *Scheduler) GetClientConfig() (bool, *config.ClientConfig) {
	// 调度器不需要提供客户端配置
	return false, nil
}
