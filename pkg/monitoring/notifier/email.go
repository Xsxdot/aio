...
type EmailNotifier struct {
	config *models.EmailNotifierConfig
	logger *zap.Logger
	id     string
	name   string
}
...
func NewEmailNotifier(notifier *models.Notifier) (Notifier, error) {
	if notifier.Type != models.NotifierTypeEmail {
		return nil, fmt.Errorf("通知器类型不是email: %s", notifier.Type)
	}

	// 解析配置
	var config models.EmailNotifierConfig
...
// Send 发送邮件通知
func (n *EmailNotifier) Send(alert *models.Alert, eventType string) (*models.NotificationResult, error) {
	result := &models.NotificationResult{
		NotifierID:   n.id,
		NotifierName: n.name,
		NotifierType: models.NotifierTypeEmail,
		Success:      false,
