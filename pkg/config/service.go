...
type Service struct {
	...
	baseConfig *config.BaseConfig // 添加对原始配置的引用
}

func (s *Service) init(config *config.BaseConfig) error {
...
func (s *Service) Start(ctx context.Context) error {
	...
	// 保存 etcd 配置到配置中心
	if err := s.saveEtcdConfigToCenter(ctx); err != nil {
		s.logger.Warn("保存etcd配置到配置中心失败", zap.Error(err))
	}
...
// NewService 创建配置中心服务
func NewService(cfg *config.BaseConfig, client *etcd.EtcdClient) (*Service, error) {
	ctx, cancel := context.WithCancel(context.Background())
	svc := &Service{
		client:     client,
		logger:     common.GetLogger().GetZapLogger("ConfigService"),
		watches:    make(map[string][]chan *ConfigItem),
		ctx:        ctx,
		cancel:     cancel,
		baseConfig: cfg, // 保存对原始配置的引用
	}

	return svc, svc.init(cfg)
}
...
func (s *Service) decryptValue(encryptedValue string) (string, error) {
	return s.decryptValueWithKey(encryptedValue, s.salt)
}

// decryptValueWithKey 使用指定密钥解密字符串值
func (s *Service) decryptValueWithKey(encryptedValue string, key []byte) (string, error) {
	// 检查并移除加密前缀
...
func (s *Service) ImportAllConfigs(ctx context.Context, encryptedData []byte, password string, skipExisting bool, originalSalt string) ([]string, error) {
	// 使用提供的密码解密数据
	s.logger.Info("开始导入配置",
		zap.Bool("skipExisting", skipExisting),
		zap.Int("数据长度", len(encryptedData)),
		zap.Bool("hasOriginalSalt", originalSalt != ""))
	...
	// 如果提供了原始Salt，创建一个临时的解密器用于解密原始加密值
	var originalDecryptFunc func(string) (string, error)
	if originalSalt != "" {
		originalKey := s.getAESKey(originalSalt)
		originalDecryptFunc = func(encryptedValue string) (string, error) {
			return s.decryptValueWithKey(encryptedValue, originalKey)
		}
	}
	...
	for _, item := range importData {
		...
		for k, v := range item.Value {
			...
			if v.Type == ValueTypeEncrypted {
				...
				// 如果提供了原始Salt，使用原始Salt解密
				if originalDecryptFunc != nil {
					// 先用原始Salt解密
					decrypted, err := originalDecryptFunc(v.Value)
					if err != nil {
						s.logger.Warn("使用原始Salt解密配置值失败",
							zap.String("key", item.Key),
							zap.String("field", k),
							zap.Error(err))
						// 如果原始Salt解密失败，尝试当前Salt
						decrypted, err = s.decryptValue(v.Value)
						if err != nil {
							s.logger.Warn("使用当前Salt解密配置值失败",
								zap.String("key", item.Key),
								zap.String("field", k),
								zap.Error(err))
							// 都失败了，保留原始值
					...
					// 没有提供原始Salt，假设值是明文或使用当前Salt加密的
					// 首先检查是否有加密前缀，如果有则解密，否则直接使用
					if strings.HasPrefix(v.Value, EncryptedPrefix) {
						decrypted, err := s.decryptValue(v.Value)
						if err != nil {
							s.logger.Warn("解密配置值失败",
								zap.String("key", item.Key),
				...
				// 使用当前Salt重新加密
				encrypted, err := s.encryptValue(plainValue)
				if err != nil {
					s.logger.Warn("重新加密配置值失败",
						zap.String("key", item.Key),
						zap.String("field", k),
						zap.Error(err))
					// 加密失败时保留明文值（这样至少数据不会丢失）
					configValue.Value = plainValue
				} else {
					// 重新加密成功
					configValue.Value = encrypted
				}
...
func (s *Service) ImportAllConfigsBase64(ctx context.Context, base64Data string, password string, skipExisting bool, originalSalt string) ([]string, error) {
	// 解码Base64数据
	...
	// 调用原有导入方法
	return s.ImportAllConfigs(ctx, data, password, skipExisting, originalSalt)
}
...
// saveEtcdConfigToCenter 将 etcd 配置保存到配置中心
func (s *Service) saveEtcdConfigToCenter(ctx context.Context) error {
	if s.baseConfig == nil || s.baseConfig.Etcd == nil {
		s.logger.Info("etcd配置为空，跳过保存到配置中心")
		return nil
	}

	etcdConfig := s.baseConfig.Etcd
	s.logger.Info("开始保存etcd配置到配置中心")
	...
	// 拨号超时配置
	configValues["dial_timeout"] = &ConfigValue{
		Value: fmt.Sprintf("%d", etcdConfig.DialTimeout),
		Type:  ValueTypeInt,
	}

	// 用户名配置
	configValues["username"] = &ConfigValue{
		Value: etcdConfig.Username,
		Type:  ValueTypeString,
	}
	...
	if etcdConfig.Password != "" {
		configValues["password"] = &ConfigValue{
			...
			Type:  ValueTypeEncrypted, // 使用加密类型
		}
	}

	// 自动同步端点配置
	configValues["auto_sync_endpoints"] = &ConfigValue{
		Value: fmt.Sprintf("%t", etcdConfig.AutoSyncEndpoints),
		Type:  ValueTypeBool,
	}
	...
	if etcdConfig.TLS != nil {
		...
		if err != nil {
			return fmt.Errorf("序列化TLS配置失败: %v", err)
		}

		configValues["tls"] = &ConfigValue{
			Value: string(tlsJSON),
	...
	metadata := map[string]string{
		"description": "etcd客户端配置",
		"auto_saved":  "true",
		"source":      "base_config",
	}

	// 保存配置到配置中心
	if err := s.Set(ctx, "etcd", configValues, metadata); err != nil {
		s.logger.Error("保存etcd配置到配置中心失败", zap.Error(err))
		return fmt.Errorf("保存etcd配置失败: %v", err)
	}

	s.logger.Info("etcd配置已成功保存到配置中心",
		zap.String("key", "etcd"),
		zap.Int("config_items", len(configValues)))
