...
type Manager struct {
	...
	rules        map[string]*models2.AlertRule
	activeAlerts map[string]*models2.Alert
...
type NotifierManager interface {
	SendAlert(alert *models2.Alert, eventType string) []models2.NotificationResult
}
...
func New(config Config, notifierManager NotifierManager) *Manager {
	...
	return &Manager{
		...
		rules:           make(map[string]*models2.AlertRule),
		activeAlerts:    make(map[string]*models2.Alert),
		notifierManager: notifierManager,
		ctx:             ctx,
		cancel:          cancel,
	}
...
func (m *Manager) loadRules() error {
	...
	// 清空现有规则
	m.rules = make(map[string]*models2.AlertRule)

	// 解析规则
	for key, value := range kvs {
		ruleID := path.Base(key)
		rule := &models2.AlertRule{}
...
func (m *Manager) evaluateAllRules() {
	m.alertsMu.RLock()
	rules := make([]*models2.AlertRule, 0, len(m.rules))
...
func (m *Manager) evaluateRule(rule *models2.AlertRule) error {
	// 解析持续时间
	...
	var queryOptions models2.QueryOptions
	queryOptions.StartTime = startTime
	queryOptions.EndTime = now

	// 根据目标类型选择查询方式
	var results *models2.QueryResult
	if rule.TargetType == models2.AlertTargetServer {
		// 查询服务器指标
		queryOptions.MetricNames = []string{string(rule.Metric)}
		results, err = m.storage.QueryTimeSeries(queryOptions)
	} else if rule.TargetType == models2.AlertTargetApplication {
		// 查询应用指标
		queryOptions.MetricNames = []string{string(rule.Metric)}
		// 添加标签匹配器
		if len(rule.LabelMatchers) > 0 {
			queryOptions.LabelMatchers = rule.LabelMatchers
		}
		results, err = m.storage.QueryTimeSeries(queryOptions)
	} else {
...
func (m *Manager) checkCondition(condition models2.AlertConditionType, value, threshold float64) bool {
	switch condition {
	case models2.ConditionGreaterThan:
		return value >= threshold
	case models2.ConditionLessThan:
		return value <= threshold
	case models2.ConditionEqual:
		return value == threshold
	case models2.ConditionNotEqual:
		return value != threshold
	default:
...
func (m *Manager) triggerAlert(alertKey string, rule *models2.AlertRule, labels map[string]string, value float64) {
	m.alertsMu.Lock()
	defer m.alertsMu.Unlock()
...
func (m *Manager) resolveAlert(alertKey string) {
	...
	now := time.Now()
	alert.State = models2.AlertStateResolved
	alert.EndsAt = &now
	alert.UpdatedAt = now
...
// GetRules 获取所有告警规则
func (m *Manager) GetRules() []*models2.AlertRule {
	m.alertsMu.RLock()
...
// GetRule 获取指定ID的告警规则
func (m *Manager) GetRule(id string) (*models2.AlertRule, error) {
	m.alertsMu.RLock()
	defer m.alertsMu.RUnlock()
...
// CreateRule 创建新的告警规则
func (m *Manager) CreateRule(rule *models2.AlertRule) error {
	if rule.ID == "" {
...
// UpdateRule 更新告警规则
func (m *Manager) UpdateRule(rule *models2.AlertRule) error {
	if rule.ID == "" {
...
func (m *Manager) GetActiveAlerts() []*models2.Alert {
	m.alertsMu.RLock()
	defer m.alertsMu.RUnlock()

	alerts := make([]*models2.Alert, 0, len(m.activeAlerts))
