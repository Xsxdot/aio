...
type WebhookNotifier struct {
	config *models2.WebhookNotifierConfig
	logger *zap.Logger
	id     string
	name   string
}
...
func NewWebhookNotifier(notifier *models2.Notifier) (Notifier, error) {
	if notifier.Type != models2.NotifierTypeWebhook {
		return nil, fmt.Errorf("通知器类型不是webhook: %s", notifier.Type)
	}

	// 解析配置
	var config models2.WebhookNotifierConfig
...
// Send 发送Webhook通知
func (n *WebhookNotifier) Send(alert *models2.Alert, eventType string) (*models2.NotificationResult, error) {
	result := &models2.NotificationResult{
		NotifierID:   n.id,
		NotifierName: n.name,
		NotifierType: models2.NotifierTypeWebhook,
		Success:      false,
