...
type WeChatNotifier struct {
	config *models2.WeChatNotifierConfig
	logger *zap.Logger
	id     string
	name   string
}
...
func NewWeChatNotifier(notifier *models2.Notifier) (Notifier, error) {
	if notifier.Type != models2.NotifierTypeWeChat {
		return nil, fmt.Errorf("通知器类型不是wechat: %s", notifier.Type)
	}

	// 解析配置
	var config models2.WeChatNotifierConfig
...
// Send 发送企业微信通知
func (n *WeChatNotifier) Send(alert *models2.Alert, eventType string) (*models2.NotificationResult, error) {
	result := &models2.NotificationResult{
		NotifierID:   n.id,
		NotifierName: n.name,
		NotifierType: models2.NotifierTypeWeChat,
		Success:      false,
