...
type Manager struct {
	...
	logger          *zap.Logger
	notifiers       map[string]*models2.Notifier
...
type NotifierFactory interface {
	...
	CreateNotifier(notifier *models2.Notifier) (Notifier, error)
}

// Notifier 通知器接口
type Notifier interface {
	// Send 发送通知
	Send(alert *models2.Alert, eventType string) (*models2.NotificationResult, error)
}
...
func New(config Config) *Manager {
	...
	return &Manager{
		...
		notifiers:       make(map[string]*models2.Notifier),
...
func (m *Manager) loadNotifiers() error {
	...
	// 清空现有配置
	m.notifiers = make(map[string]*models2.Notifier)

	// 解析配置
	for key, value := range kvs {
		notifierID := path.Base(key)
		notifier := &models2.Notifier{}
...
// SendAlert 发送告警通知
func (m *Manager) SendAlert(alert *models2.Alert, eventType string) []models2.NotificationResult {
	if alert == nil {
	...
	results := make([]models2.NotificationResult, 0, len(notifiers))
...
func (m *Manager) getEnabledNotifiers() []*models2.Notifier {
	m.notifiersMu.RLock()
	defer m.notifiersMu.RUnlock()

	notifiers := make([]*models2.Notifier, 0)
...
func (m *Manager) GetNotifiers() []*models2.Notifier {
	m.notifiersMu.RLock()
	defer m.notifiersMu.RUnlock()

	notifiers := make([]*models2.Notifier, 0, len(m.notifiers))
	for _, notifier := range m.notifiers {
		notifiers = append(notifiers, notifier)
	}
	return notifiers
...
func (m *Manager) GetNotifier(id string) (*models2.Notifier, error) {
	m.notifiersMu.RLock()
	defer m.notifiersMu.RUnlock()
...
func (m *Manager) CreateNotifier(notifier *models2.Notifier) error {
	// 检查通知器类型是否受支持
...
func (m *Manager) UpdateNotifier(notifier *models2.Notifier) error {
	if notifier.ID == "" {
...
func isSupportedNotifierType(t models2.NotifierType) bool {
	supportedTypes := []models2.NotifierType{
		models2.NotifierTypeEmail,
		models2.NotifierTypeWebhook,
		models2.NotifierTypeWeChat,
		models2.NotifierTypeDingTalk,
	}
...
// CreateNotifier 创建通知器实例
func (f *DefaultNotifierFactory) CreateNotifier(notifier *models2.Notifier) (Notifier, error) {
	switch notifier.Type {
	case models2.NotifierTypeEmail:
		return NewEmailNotifier(notifier)
	case models2.NotifierTypeWebhook:
		return NewWebhookNotifier(notifier)
	case models2.NotifierTypeWeChat:
		return NewWeChatNotifier(notifier)
	case models2.NotifierTypeDingTalk:
