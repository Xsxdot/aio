...
// QueryServiceRequest 查询 service 列表请求
...
// ControlServiceRequest 控制 service 请求（启动/停止/重启/重载）
type ControlServiceRequest struct {
	Action string `json:"action" validate:"required,oneof=start stop restart reload" comment:"操作类型"`
}

// EnableServiceRequest 启用/禁用 service 请求
type EnableServiceRequest struct {
	Enable bool `json:"enable" comment:"true 启用，false 禁用"`
}
...
// ServiceUnitParams systemd service unit 参数（结构化 + 扩展行）
...
// GenerateServiceRequest 生成 service unit 内容请求（仅预览，不落盘）
type GenerateServiceRequest struct {
	Params ServiceUnitParams `json:"params" validate:"required" comment:"unit 参数"`
}

// GenerateServiceResponse 生成 service unit 内容响应
type GenerateServiceResponse struct {
	Content string `json:"content" comment:"生成的 unit 文件内容"`
}

// CreateServiceFromParamsRequest 按参数创建 service 请求
type CreateServiceFromParamsRequest struct {
	Name   string            `json:"name" validate:"required,max=200" comment:"服务名称（需以 .service 结尾）"`
	Params ServiceUnitParams `json:"params" validate:"required" comment:"unit 参数"`
}

// UpdateServiceFromParamsRequest 按参数更新 service 请求
type UpdateServiceFromParamsRequest struct {
	Params ServiceUnitParams `json:"params" validate:"required" comment:"unit 参数"`
}
